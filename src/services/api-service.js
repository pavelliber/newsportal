export default class APIService {
    ...
    _fakeContent = 'China is facing mounting criticism over its plan to impose a security law on Hong Kong which would make it a crime to undermine Beijing\'s authority in the territory.\n' +
    '\n' +
    ...
    // async for future real server
    getMainNewsList = async () => {
    ...
    getArticleContent = async (id= 1) => {
        const item = this._searchResults.find(obj => obj.id === id.toString());
        if (!item) return null;
        return {
            topic: item.topic,
            text: item.text,
            img: item.img,
            content: this._fakeContent
        };
    };

    getLimitedArticleContent = async (id= 1) => {
        const item = this._searchResults.find(obj => obj.id === id.toString());
        if (!item) return null;
        return {
            topic: item.topic,
